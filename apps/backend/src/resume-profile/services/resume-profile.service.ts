...
export class ResumeProfileService {
  constructor(private readonly resumeProfileRepository: ResumeProfileRepository) { }
  ...
  async create(createResumeProfileDto: CreateResumeProfileDto): Promise<ResumeProfile> {
    ...
    return await this.resumeProfileRepository.create(createResumeProfileDto);
  }


  async findById(id: number): Promise<ResumeProfile> {
    const profile = await this.resumeProfileRepository.findById(id);
    if (!profile) {
  ...
  async findByAccountId(accountId: number): Promise<ResumeProfile> {
    const profile = await this.resumeProfileRepository.findByAccountId(accountId);
    if (!profile) {
  ...
  async update(id: number, updateResumeProfileDto: UpdateResumeProfileDto): Promise<ResumeProfile> {
    ...
    const existingProfile = await this.resumeProfileRepository.findById(id);
    if (!existingProfile) {
      throw new NotFoundException(`Resume profile with ID ${id} not found`);
    }
    ...
    const updatedProfile = await this.resumeProfileRepository.update(id, updateResumeProfileDto);
    if (!updatedProfile) {
      throw new NotFoundException(`Resume profile with ID ${id} not found`);
    }
