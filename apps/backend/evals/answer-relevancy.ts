...
// Relevancy scoring prompt from Mastra
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object: statementsResult } = await generateObject({
    ...
    schema: z.object({
      statements: z
        .array(z.string())
        .describe("Array of meaningful statements from the output"),
    }),
  });

  // Handle edge case of empty statements
  const statements =
      ...
      verdicts: z
        .array(
          z.object({
            ...
            reason: z.string().describe("Explanation of the verdict"),
          })
        )
        .describe("Array of verdict objects for each statement"),
    }),
  });

  console.log("Verdict results:", verdictResult.verdicts);

  // Step 3: Convert verdicts to scores and average them
  const scores = {
    yes: 1,
    no: 0,
    unsure: 0.5,
  ...
  const statementScores = verdictResult.verdicts.map(
